...
def PO_gen():
    ...
    if request.method == 'POST': 
        ...
        + "\noffice_chair(" + str(office_chair.quantity) + " no's): $" + str(office_chair.total) + "\nTablet(" + str(tablet.quantity) \
...
def write_to_dbs():
   ...
   (db, cursor) = db_conn()
   (pub, priv) = rsa.newkeys(LEN_OF_KEY)   #generate public and private keys
   pu = pickle.dumps(pub)           #convert public key object into string - pickling
   pr = pickle.dumps(priv)        #convert private key object into string - pickling
   cursor.execute("""INSERT INTO Credential VALUES (%s,%s,%s)""", (EMAIL, PASSWORD, pr))    #insert credentials
   db.commit()
   cursor.execute("""INSERT INTO PU_Directory VALUES (%s,%s)""", (EMAIL, pu))         #insert public key in public key directory 
   db.commit()
   cursor.close()
   db.close()
   return redirect(url_for('PO_gen'))
...
def check_pass():
   ...
   password = cursor.fetchone()              #fetch password
   cursor.close()
   db.close()
   if (password == None):            #no entry in db
